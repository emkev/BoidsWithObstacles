...
*/
  ...
  /* 2016.11.15 
     ...
     obs , start
  */
  PVector avoid_obstacle(ArrayList<Obstacle> obs)
  {
    float action_check_dist = 70 ;
    PVector sum = new PVector(0 , 0);
    int count = 0 ;
    
    for(Obstacle ob : obs) {

      float dist = PVector.dist(location , ob.location);
    
      if( (dist > 0) && (dist < action_check_dist) ) {
        PVector action_vec = PVector.sub(location , ob.location);
  ...
  void flock(ArrayList<boid> boids , ArrayList<Obstacle> obs)
  {
    ...
    /* obs , start */
    //PVector obs = new PVector(100 , 100);
    //PVector mouse_obs = new PVector(mouseX , mouseY) ; 
    PVector avoid_obs = avoid_obstacle(obs); 
    /* obs , end */
